...
Agent Paw(Product Agentic Workflow)는 제품 기획, 시스템 설계, UI/UX 디자인, 소스 코드 구현 및 검증의 전 과정을 복수의 특화 AI 모델(Claude, Gemini)을 통해 통합 수행하는 일렉트론 기반의 지능형 개발 에이전트 시스템이다.
...
연관 시스템: Claude API, Gemini API, Claude Code CLI, Google Space API, Git.
핵심 테이블: PROJECT, PROJECT_MEMBER, WORKSPACE, PERSONA, SNAPSHOT, EVENT_LOG, WIKI_DOCUMENT, USER, AUTH_TOKEN, AUDIT_LOG.
...
### 4.9. 개발 모드 Chromium 캐시 자동 초기화 정책

Electron의 Chromium HTTP 캐시가 이전 번들을 서빙하여 UI가 stale 상태로 표시되는 문제를 방지하기 위해 개발 모드에서 캐시를 자동 초기화한다.

| 항목 | 정책 |
|---|---|
| 적용 조건 | `app.isPackaged === false` (개발 모드)인 경우에만 실행한다 |
| 실행 시점 | `app.on('ready')` 이벤트 핸들러에서 DB 초기화 이전에 실행한다 |
| 초기화 대상 | `session.defaultSession.clearCache()` — Chromium HTTP 캐시 및 Code Cache |
| 프로덕션 모드 | 프로덕션 빌드에서는 자동 초기화를 수행하지 않는다. User가 설정 화면의 "캐시 삭제 및 새로고침" 버튼(§13.3)을 통해 수동으로 삭제한다 |
...
| 4. 비동기 물리 정리 | 논리 삭제 완료 후 System이 백그라운드 작업을 시작한다 | - | Server가 순차적으로 연관 데이터를 물리 삭제한다: (1) WIKI_DOCUMENT (2) EVENT_LOG (3) SNAPSHOT + Vector DB 인덱스 (4) AUDIT_LOG 중 project_id 일치 기록 (5) PERSONA (6) WORKSPACE (7) PROJECT_MEMBER | 각 테이블 DELETE |
...
연관 시스템: Claude API, Gemini API, Claude Code CLI, Local Git, Vector DB.
...
외부에서 구글 스페이스 채팅을 통해 Agent Paw PC 앱에 명령을 전달하고 결과를 수신하는 원격 제어 모듈이다.
...
| [Button] 캐시 삭제 및 새로고침 | N | 앱 정보 영역에 배치한다. 클릭 시 System이 Chromium HTTP 캐시 및 스토리지 캐시(appcache, cachestorage)를 삭제하고 Client를 강제 새로고침(reloadIgnoringCache)한다. 완료 시 "캐시 삭제 완료 — 재시작됩니다" 메시지를 출력한다 |
