...
|---|---|---|---|---|---|
| 기획 | Claude Sonnet | Gemini 1.5 Pro | User 요구사항, 레거시 문서 | PRD, 유즈케이스, User Journey Map | Gemini Pro → Claude Opus |
| 설계 | Claude Sonnet | - | PRD, 도메인 엔티티 | ERD, API 설계서, IaC 초안 | Claude Opus |
| 시각 | Midjourney | Gemini Flash | 브랜드 키워드, 와이어프레임 텍스트 | UI 목업 이미지 → Tailwind CSS + React 컴포넌트 | Gemini Flash 단독 |
| 구현 | Llama 3 (로컬) | Claude Sonnet | 설계서, 컴포넌트 구조 | 소스 코드, 폴더 구조, 단위 테스트 | Claude Sonnet 단독 |
...
주요 접근 대상: Owner, Editor, Viewer.
연관 시스템: Google OAuth 2.0, Google UserInfo API, Google Space API, OS Keychain.
...
| UI 컴포넌트 | 필수 | Validation / 시스템 동작 |
|---|---|---|
| [Input] Claude API Key | N | 마스킹 출력(`sk-****...****1234`). Owner만 편집 가능. 입력 시 System이 즉시 테스트 요청으로 유효성을 검증한다 |
...
| [Button] 구글 스페이스 연결 | N | Owner만 활성화. Google Space API OAuth 동의 화면으로 리다이렉트한다. scope: spaces.messages.create, spaces.messages.read. 연결 성공 시 System이 AUDIT_LOG에 SPACE_CONNECT 이벤트를 기록한다 |
| [Button] 구글 스페이스 연결 해제 | N | 연결된 상태에서만 노출. System이 AUTH_TOKEN에서 Google Space Refresh Token을 삭제하고 리모트 제어 기능을 비활성화한다. System이 AUDIT_LOG에 SPACE_DISCONNECT 이벤트를 기록한다 |
