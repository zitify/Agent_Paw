...
public partial class AuthService
{
    ...
    public (string UserId, string Email)? VerifyToken(string token)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[AuthService] VerifyToken failed: {ex.Message}");
            return null;
        }
    }
    ...
    public async Task<(string Token, User User)> DevBypassLoginAsync()
    {
        ...
        {
            throw new InvalidOperationException(
                "DevBypassLoginAsync is disabled. Set DevBypass=true and AGENTPAW_ALLOW_DEV_BYPASS=true only for local development.");
        }
